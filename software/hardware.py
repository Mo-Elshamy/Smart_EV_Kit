...
try:
    ...
    # Added 'ws' to imports for strip type configuration
...
class LightingSystem:
    def __init__(self):
        # UPDATED: Added 'strip_type=ws.WS2811_STRIP_RGB' to fix color swap
        self.strip = PixelStrip(LED_TOTAL, PIN_LED_DATA, 800000, 10, False, 255, 0, ws.WS2811_STRIP_RGB)
        if IS_RASPBERRY_PI:
            self.strip.begin()
    ...
    def _render_front(self, blink):
        ...
        for i in range(start, end):
            if i < start + 1 and self.turn_signal_state in ["LEFT", "HAZARD"] and blink:
                self.strip.setPixelColor(i, Color(255, 150, 0))
            elif i >= end - 1 and self.turn_signal_state in ["RIGHT", "HAZARD"] and blink:
                self.strip.setPixelColor(i, Color(255, 150, 0))
            else:
    ...
    def _render_back(self, blink):
        ...
        base_color = Color(255, 0, 0) if self.brake_active else Color(255, 0, 0) 
        
        for i in range(start, end):
            if i < start + 2 and self.turn_signal_state in ["LEFT", "HAZARD"] and blink:
                self.strip.setPixelColor(i, Color(255, 150, 0))
            elif i >= end - 2 and self.turn_signal_state in ["RIGHT", "HAZARD"] and blink:
                self.strip.setPixelColor(i, Color(255, 150, 0))
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            car.trigger_sensors()
            time.sleep(0.5)
            print(f"Distances: {car.distances}")
